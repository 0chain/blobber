...
echo ""
echo "3> docker build"
DOCKER_BUILDKIT=1 docker $DOCKER_BUILD --progress=plain --buildkitd-flags '--allow-insecure-entitlement security.insecure --allow-insecure-entitlement network.host' --build-arg GIT_COMMIT=$GIT_COMMIT -f docker.local/base.Dockerfile . -t $DOCKER_IMAGE_BASE
