...
echo ""
echo "3> docker build"
DOCKER_BUILDKIT=1 docker $DOCKER_BUILD --buildkitd-flags '--allow-insecure-entitlement network.host' --progress=plain --build-arg GIT_COMMIT=$GIT_COMMIT -f docker.local/base.Dockerfile . -t $DOCKER_IMAGE_BASE --network none
