...
sudo apt update
sudo apt install -y build-essential jq wget unzip curl docker.io
...
services:
  ...
  validator:
    ...
    ports:
      - "5061:31401"
    command: ./bin/validator --port 31401 --hostname ${CLUSTER}.${DOMAIN} --deployment_mode 0 --keys_file keysconfig/b0bnode01_keys.txt --log_dir /validator/log
    networks:
      default:
      testnet0:
