...
func (nf *DeleteFileChange) CommitToFileStore(ctx context.Context) error {
	...
	for contenthash := range nf.ContentHash {
		var count int64
		err := db.Table((&reference.Ref{}).TableName()).Where(&reference.Ref{ThumbnailHash: contenthash}).Or(&reference.Ref{ContentHash: contenthash}).Where("deleted_at IS null").Count(&count).Error
		if err == nil && count == 0 {
