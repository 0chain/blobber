	...
	for _, tc := range testCases {
		...
		t.Run(tc.name, func(t *testing.T) {
			_, err := func() (*reference.Ref, error) {
				resp, err := change.ApplyChange(ctx, tc.allocChange, tc.allocRoot, common.Now()-1, nil)
				if err != nil {
					return nil, err
				}
