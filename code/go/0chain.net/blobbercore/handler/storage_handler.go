...
func (fsh *StorageHandler) AddCommitMetaTxn(ctx context.Context, r *http.Request) (interface{}, error) {
	...
	if clientID != allocationObj.OwnerID {
		...
		if authTokenString == "" {
			return nil, common.NewError(PermissionDenied, "client has no permission")
		}

		authToken, err := fsh.verifyAuthTicket(ctx, r.FormValue("auth_token"), allocationObj, fileref, clientID)
		if authToken == nil {
			return nil, common.NewError("auth_ticket_verification_failed", fmt.Sprintf("Could not verify the auth ticket. %v", err))
		}
	}
