...
func (cmd *UpdateFileCommand) IsValidated(ctx context.Context, req *http.Request, allocationObj *allocation.Allocation, clientID string) error {
	...
	// set to default values , should not be provided by client
	cmd.fileChanger.PrevThumbnailHash = ""
	cmd.fileChanger.PrevValidationRoot = ""
...
func (cmd *UpdateFileCommand) ProcessRollback(allocationObj *allocation.Allocation, connectionObj *allocation.AllocationChangeCollector) error {
	...
	fileSize, thumbSize, err := filestore.GetFileStore().GetFilePathSize(allocationObj.ID, cmd.fileChanger.ValidationRoot, cmd.existingFileRef.PrevThumbnailHash)

	if err != nil {
		return common.NewError("file_not_found", "File not found")
	}

	allocationSize += (fileSize - cmd.existingFileRef.Size)

	connectionObj.Size = allocationSize

	cmd.fileChanger.Size = fileSize
	cmd.fileChanger.ThumbnailSize = thumbSize
	cmd.fileChanger.ThumbnailHash = cmd.existingFileRef.PrevThumbnailHash
